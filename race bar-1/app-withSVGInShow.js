...
for (var j = 0; j < playerNames.length; j++) {
    playersPosition[j] = createRect();
    console.log(playersPosition[j]);

    //   playersPosition[j].style.position = 'absolute';
    playersPosition[j].fill = playerColors[j];
...
setInterval(function d() {
    ...
    if (i1 < data.length) {
        ...
        for (var i = indexData.length - 1, j = 1; i >= 0; i--, j++) {
            ...
            playersPosition[indexData[i]].height = '20px';
            //   playersPosition[indexData[i]].style.width=copy2[i]+'px';

            let str = playersPosition[indexData[i]].y;
            let slicedStr;
            if (str) { slicedStr = str.slice(0, playersPosition[indexData[i]].y.length - 2); }
            ...
            if (slicedStr) {
                ...
                playersPosition[indexData[i]].y = j * initialTopPosition * 10 + 'px';
            }
        }
        i1++;
    }
