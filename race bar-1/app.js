        ...
        if(i1<data.length){
            ...
            for(var j=indexData.length-1,p=1;j>=0;j--,p++){
             ...
             //   players[indexData[j]].style.width=copy2[j]+'px';
                ...
                let str=players[indexData[j]].style.top;let slicedStr;
                if(str){slicedStr=str.slice(0,players[indexData[j]].style.top.length-2);}
                
               let neededToFit=p*initialTopPosition*10;
                
            if(slicedStr){    
               if(slicedStr===neededToFit){ $(players[indexData[j]]).animate({"width":`${copy2[j]}px`},500,"swing");  continue;}
                else if(slicedStr>neededToFit){
                    $(players[indexData[j]]).animate({"width":`${copy2[j]}px`},500,"swing");  
                    $(players[indexData[j]]).animate({"top":`${neededToFit}px`},500,"swing");
